...
info1 <- read_delim("S:/pools/n/N-IUNR-nova-data/06_add_var/01_dokumentation/final/menu_inhalt_protein_180420_egel07_final.csv", delim=';',locale = locale(encoding = 'LATIN1'), col_types = cols(date=col_datetime(format = "%d.%m.%Y"))) # should be the final version :) 
...
envir_nutri <- read_delim("S:/pools/n/N-IUNR-nova-data/02_kassendaten/00_original/Versand an ZHAW/files/envir_nutri_tot_wide_180613_egel.csv", delim = ';',locale = locale(encoding = "LATIN1"),
                          col_types = cols(
                              ...
                              gwp_tot = col_number())) %>%
    mutate(date=as.POSIXct(date, format="%Y-%m-%d", tz="CET")) %>%
    select(-X1)
...
info_compl <- left_join(info1, envir_nutri, by=c("article_description","date", "cycle")) %>%
...
# save dataset
#write_delim(df_7, "S:/pools/n/N-IUNR-nova-data/02_kassendaten/00_original/Versand an ZHAW/files/r_final_180503_02egel.csv", delim=';')
