...
df_R <- filter(df_17, art_code %in% articles) # no time filter
df_PY <- filter(df_17_py, art_code %in% articles)# also hear, results into another data frame => reasons not same structure

#unique values in transaction id's => 70 missing cases
length(unique(df_R$transaction_id))
length(unique(df_PY$transaction_id))


# search for difference => 70 cases in garden are missing => also hear problem of merging ZHAW_articles in python
