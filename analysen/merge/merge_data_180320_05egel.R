...
trans_dat <- read_delim("S:/pools/n/N-IUNR-nova-data/02_kassendaten/00_original/Versand an ZHAW/ZHAW_Transactions_180320.csv",delim=';',col_names = T,trim_ws = T)# trim_ws removes white space
...
# location of shop
shop <- read_excel("S:/pools/n/N-IUNR-nova-data/02_kassendaten/00_original/Versand an ZHAW/ZHAW_Shops.xlsx")# good for small datasets
...
df_a <- read_delim("S:/pools/n/N-IUNR-nova-data/02_kassendaten/00_original/Versand an ZHAW/ZHAW_Trans_Articles.csv",delim = ';',trim_ws = T) # throw back errors, however reading it with csv2 same results 
...
df_ai <-  read_excel("S:/pools/n/N-IUNR-nova-data/02_kassendaten/00_original/Versand an ZHAW/ZHAW_Articles.xlsx")
df_ai <- select(df_ai, id, code, description)
df_ai <- rename(df_ai, article_id=id, art_code = code, art_description = description)

# load price levels
price_lev <- read_excel("S:/pools/n/N-IUNR-nova-data/02_kassendaten/00_original/Versand an ZHAW/ZHAW_Pricelevels.xlsx")
...
df_ap <- read_excel("S:/pools/n/N-IUNR-nova-data/02_kassendaten/00_original/Versand an ZHAW/ZHAW_prices.xlsx") # conversts some variables into characters => reasons?
...
df_p <- read_delim("S:/pools/n/N-IUNR-nova-data/02_kassendaten/00_original/Versand an ZHAW/ZHAW_Trans_Payments.csv",delim=';',trim_ws = T)
...
df_p2 <- read_delim("S:/pools/n/N-IUNR-nova-data/02_kassendaten/00_original/Versand an ZHAW/ZHAW_payments.csv",delim = ';',trim_ws = T)
...
df_pay <- read_delim("S:/pools/n/N-IUNR-nova-data/02_kassendaten/00_testdaten_original/Versand an ZHAW 180103/ZHAW_payments.csv", delim = ';',trim_ws = T)
...
df1 <- inner_join(df,df_a, by="transaction_id") 

# merge df1 with atricle information
df2 <- inner_join(df1, df_ai, by="article_id")
...
df4 <- inner_join(df3,pay_inf, by="transaction_id") # some new observations came along => why
...
write_delim(df_dat,"S:/pools/n/N-IUNR-nova-data/02_kassendaten/00_original/Versand an ZHAW/data_180320_05egel.csv",delim=';')
