...
# plot in en
ggplot(df_t, aes(y=tot,x=xlab, fill=factor(label_content,levels=c("Vegan","Vegan+","Vegetarian","Meat","Hot and Cold")), label=format(df_t$tot, big.mark = "'", scientific = F))) +
    geom_bar(stat="identity", position = "stack") +
...
ggsave("S:/pools/n/N-IUNR-nova-data/02_kassendaten/01_analysen/ist soll analyse/plots/actual_target_comp_condition_de 2017 180615 egel.png",
       width = 14,
       height = 8,
       dpi=600,
       units="in",
...
df_ist=group_by(df_7,date,article_description,label_content,shop_description, condit)%>% summarise(tot_sold=n())

# Target: locals are included
df_soll=read_excel("S:/pools/n/N-IUNR-nova-data/06_add_var/02_planung_verkaufszahlen/Mensa_geplant_180216_02egel.xlsx")
df_soll=rename(df_soll, article_description=Menülinie, date=Datum, shop_description= Ort)
df_soll$shop_description=str_replace(df_soll$shop_description,"Grüental","Grüental Mensa")
df_soll$shop_description=str_replace(df_soll$shop_description,"Vista","Vista Mensa")
...
df_t$offer <- str_replace(df_t$offer,"Actual","Ist")
df_t$xlab <- paste(df_t$offer,df_t$condit, sep = "\n")
colours <- c("Unknown" = "black","Vegan"="black", "Vegan+"="#ffffff", "Vegetarian" = "#ffffff", "Meat" = "#ffffff","Hot and Cold"="#ffffff")


ggplot(df_t, aes(y=tot,x=xlab, fill=factor(label_content,levels=c("Unknown","Vegan","Vegan+","Vegetarian","Meat","Hot and Cold")))) +
    geom_bar(stat="identity", position = "stack") +
    xlab("\nIst-Soll-Vergleich zwischen Basis- und Interventionswochen") + 
    ylab("Geplante und tatsächliche Menüverkäufe im 2017 ")+ 
    #     ggtitle("note: locals are not included
    ...
    guides(fill=guide_legend("Menü-Inhalt\n",
                             ...
                             default.unit="inch"))+
    # scale_y_continuous(labels = scales::percent)+
    scale_fill_manual(values = c("Unknown" = "#262626","Vegan"="grey90", "Vegan+"="#80ccff", "Vegetarian" = "#c5b87c", "Meat" = "#fad60d","Hot and Cold"="#262626"),
                      breaks = c("Unknown","Vegan","Vegan+","Vegetarian","Meat","Hot and Cold"),
                      labels = c("Unbekannt","Pflanzlich","Pflanzlich+","Vegetarisch","Fleisch", "Hot and Cold"))+
    scale_x_discrete(limits=c("Soll\nBasis","Ist\nBasis","Soll\nIntervention","Ist\nIntervention"))+
    geom_text(aes(label=ifelse(df_t$tot>180,format(df_t$tot, big.mark = "'", scientific = F),"")),size = 5, position = position_stack(vjust = 0.5), color="white")+ #omit numbers to be shown smaller than 190
    # scale_colour_manual(values=colours)+ # try print all vegan numbers black others white, not working
    annotate("text",x=1:4,y=13700,label=c(lbt$label2[3],lbt$label2[1],lbt$label2[4],lbt$label2[2]), size=6)+
    mytheme


ggsave("S:/pools/n/N-IUNR-nova-data/02_kassendaten/01_analysen/ist soll analyse/plots/actual_target_comp_condition_mitlocals_de 2017 180615 egel.png",
       width = 14,
       height = 8,
       dpi=600,
       units="in",
...
##prepare data
# target for 2015 and 2016 and 2017 for both canteens (per canteen 240 meals), and for both cycles (2x)
...
df_ist_7=group_by(df_7,date,year,article_description,label_content,shop_description, condit) %>%
    ...
    select(year, label_content, shop_description) %>% # dont work
    mutate(offer = "Ist")

#check how many offers over both cycle
meal_offer <- df_ist_7 %>%
    group_by(article_description, shop_description) %>%# count all meal offers in 2017 over both canteens
    summarise(tot_offer=n())

# select is not working dont know why    
df_ist_7$date <- NULL
df_ist_7$article_description <- NULL
df_ist_7$condit <- NULL
...
#plot
ggplot(df_, aes(y=tot,x=xlab, fill=factor(label_content,levels=c("Unknown","Vegan","Vegan+","Vegetarian","Meat","Hot and Cold")))) +
    geom_bar(stat="identity", position = "stack") +
    ...
    scale_fill_manual(values = c("Unknown" = "#262626","Vegan"="grey90", "Vegan+"="#80ccff", "Vegetarian" = "#c5b87c", "Meat" = "#fad60d","Hot and Cold"="#262626"),
                      ...
                      labels = c("Unbekannt","Pflanzlich","Pflanzlich+","Vegetarisch","Fleisch", "Hot and Cold"))+
    scale_x_discrete(limits=c("Soll\n2015-2016","Soll\n2017","Ist\n2017"))+
    geom_text(aes(label=ifelse(tot>10,tot,"")),size = 5, position = position_stack(vjust = 0.5), color="white")+ #omit numbers to be shown smaller than 190
    # scale_colour_manual(values=colours)+ # try print all vegan numbers black others white, not working
    annotate("text",x=1:3,y=c(510,510,620),label=c(text$label2[1],text$label2[1],text$label2[2]), size=6)+
    mytheme

# save plot
ggsave("S:/pools/n/N-IUNR-nova-data/02_kassendaten/01_analysen/ist soll analyse/plots/actual_target_comp_menu_offer 2015-2017 180615 egel.png",
       width = 14,
       height = 8,
       dpi=600,
       units="in",
...
df_ist_7=group_by(df_7,date,year,article_description,shop_description,label_content, condit) %>%
    summarise(tot_sold = n())%>% 
    # select(year, label_content, condit) %>% # dont work
    mutate(offer = "Ist")

# select is not working dont know why    
df_ist_7$date <- NULL
df_ist_7$article_description <- NULL
df_ist_7$tot_sold <- NULL
df_ist_7$shop_description <- NULL
