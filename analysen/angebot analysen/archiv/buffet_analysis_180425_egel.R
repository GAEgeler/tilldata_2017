...
p <- ggplot(df_, aes(reorder(as.factor(df_$xlab)),tot, fill= shop_description))+
    ...
    geom_text(aes(label=tot),position = position_dodge(width = 1), vjust = -0.2, size=8)+
    mytheme
...
p + labs(caption="Quelle: ZHAW, Kassendaten SV Schweiz", subtitle = "Stand: 13.7.18")
