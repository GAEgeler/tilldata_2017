...
ubp <- read_xlsx("S:/pools/n/N-IUNR-nova-data/09_bewertung_umwelt/Auswertung/Umweltbewertung_Menusv0.21.xlsx", range="Rezepte!D4:T96", trim_ws=TRUE, col_names = F) %>% # load specific space in excel sheet
    rename(meal_name = X__1, article_description = X__2, week = X__3, ubp_ingred = X__9,  carbs = X__10,  protein = X__11,   veg_fruit = X__12, oil_fat_nut = X__13,  sweet_salt = X__14,  beverag = X__15, foodwaste = X__16, kitchen = X__17, ubp_total) %>% # rename variables
...
gwp <- read_xlsx("S:/pools/n/N-IUNR-nova-data/09_bewertung_umwelt/Auswertung/Umweltbewertung_Menusv0.21.xlsx", range="Rezepte!AD4:AL96", trim_ws=TRUE, col_names = F) %>%
    rename(meal_name = X__1, total = X__2,  carbs = X__3,  protein = X__4,   veg_fruit = X__5, oil_fat_nut = X__6,  sweet_salt = X__7,  beverag = X__8, kitchen = X__9) %>%
...
envir_wide <- read_xlsx("S:/pools/n/N-IUNR-nova-data/09_bewertung_umwelt/Auswertung/Umweltbewertung_Menusv0.21.xlsx", range="Rezepte!D4:AM96", trim_ws=TRUE, col_names = F) %>% # load specific space in excel sheet
    rename(meal_name = X__1, article_description = X__2, week = X__3, ubp_tot_ingred = X__9,  ubp_carbs = X__10,  ubp_protein = X__11,   ubp_veg_fruit = X__12, ubp_oil_fat_nut = X__13,  ubp_sweet_salt = X__14,  ubp_beverag = X__14, ubp_foodwaste = X__16,  ubp_kitchen = X__17, gwp_tot_ingred = X__9,  gwp_carbs = X__10,  gwp_protein = X__11,   ubp_veg_fruit = X__12, gwp_oil_fat_nut = X__13,  ubp_sweet_salt = X__14,  gwp_beverag = X__14, gwp_foodwaste = X__16,  gwp_kitchen = X__17) %>% # rename variables
    mutate(cycle = 1, method = "ubp", X__4=str_sub(X__4, end = -2)) %>% # create some new variables, like cycle, calculation method, date
    mutate(date=paste(str_sub(X__4, end=-3), str_sub(X__4, start=4),17, sep = ".")) %>% # paste date together, however need some changes
    select(meal_name, article_description, date, week, cycle, method, carbs,  protein,   veg_fruit, oil_fat_nut,  sweet_salt,  beverag,  kitchen, total) ## select variables
