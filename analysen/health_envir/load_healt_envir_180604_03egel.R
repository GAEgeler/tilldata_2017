...
nutri_wide <- read_xlsx("S:/pools/n/N-IUNR-nova-data/08_bewertung_gesundheit/Zusammenfassung_EBP_Tellermodell_V6.xlsx", range = "Tabelle1!A6:H99", trim_ws = T, col_names = F) %>%
    ...
    mutate(article_description = ifelse(.$meal_line == "F","Favorite", ifelse(.$meal_line == "K","Kitchen", ifelse(.$meal_line == "G","World","no_info")))) %>%
    mutate(date = sub("\\.\\.",".",.$date)) %>% # replace double dot
    mutate(date = as.POSIXct(.$date, format = "%d.%m.%y", tz = "UTC")) %>% # convert to date format for merging with documentation, PAY ATTENTION to date format for merging, causes severe problems
    mutate(cycle = 1) %>%
...
nutri_wide[(nutri_wide$article_description == "Favorite") & (grepl("Quornragout",nutri_wide$meal_name)),]$date <- "2017-11-10 UTC" # changes date in CET (that means after the time is 2017-11-09 23:00:00) => change date to UTC, however is not working
nutri_wide[grep("Hot", nutri_wide$meal_name),]$date <- "2017-11-09 00:00:00 CET"

nutri_wide <- nutri_wide[-grep("85_Auberginen-Moussaka_45K9.11",nutri_wide$meal_name),] # delete moussaka with meat 
nutri_wide[grep("85_Auberginen-Moussaka",nutri_wide$meal_name),]$date <- "2017-11-09 00:00:00 UTC" # change date to correct date in cycle 1

#nutri_wide$date <- ymd_hms(nutri_wide$date, tz="UTC+1")

# save data
write_delim(nutri_wide,"S:/pools/n/N-IUNR-nova-data/02_kassendaten/00_original/Versand an ZHAW/files/nutri_profil_wide_180612_egel.csv", delim = ';')
