...
df_tot=read_delim("all data over year 180424 02egel.csv",delim = ';')

df_tot$value <- as.integer(df_tot$value)
df_tot$X1 <- NULL
df_tot <- filter(df_tot, week >= 40 & week <=51)
df_tot$year <- as.character(df_tot$year)
pattern = grepl("\\*",df_tot$variable) # not working => dont know why
test <- gsub(pattern,"Vegan",df_tot$variable)
...
leveneTest(df_tot$value~df_tot$variable) # nicht homogene gruppen
