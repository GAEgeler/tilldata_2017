...
# PAY ATTENTION to date format for merging
...
df_new1$date <- as.Date(df_new1$date) # change date format from POSIXct to Date

# load documentation, buffet_information and data to nutritional profiles and environmental impact -----------
info1 <- read_delim("S:/pools/n/N-IUNR-nova-data/06_add_var/01_dokumentation/final/menu_inhalt_protein_180420_egel07_final.csv", delim=';',locale = locale(encoding = 'LATIN1'), col_types = cols(date=col_date(format = "%d.%m.%Y"))) # pay attention to the date format (now is it Date format not POSIXct)
...
buffet <- read_xlsx("S:/pools/n/N-IUNR-nova-data/06_add_var/01_dokumentation/buffet_animal_180425_03egel.xlsx") %>%
    mutate(article_description = "Hot and Cold") %>%
    mutate(date = parse_date(.$date)) # to get same date format as other data frames

# merge documentation with data to nutrition 
info_compl <- left_join(info1, envir_nutri, by=c("article_description","date", "cycle", "week")) %>%
    select(-meal_name.y,-`Kein Protein`,-Kommentar, -content, -week) %>%
    rename(meal_name = meal_name.x)
info_compl <- left_join(info_compl, buffet, by=c("date","article_description","shop_description")) # some cases are missing => why?

# merge documentation with till data 
info <- select(info1, date,article_description,label_content,cycle,meal_name,shop_description) # subset of info_compl
info_ <- select(info_compl, date,article_description,label_content,cycle,meal_name,shop_description, gwp_tot, ubp_tot, ebp_points, ebp_label, teller_points, teller_label, buffet_animal_comp) # another subset of info_compl
df_17 <- left_join(df_new1,info, by = c("shop_description","date","article_description","cycle"))
df_17_ <- left_join(df_new1,info_, by = c("shop_description","date","article_description","cycle"))
...
# # remove some data from workingspace
# rm(list=c())

# save dataset
write_delim(df_7, "S:/pools/n/N-IUNR-nova-data/02_kassendaten/00_original/Versand an ZHAW/files/r_final_180503_03egel.csv", delim=';')
