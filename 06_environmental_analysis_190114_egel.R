...
# load data
source("05_1_load_add_data_190128_egel.R") # do only need ubp_ and gwp_1, attention geflügel as new category at label_content
...
ubp <- ubp_ %>% 
    mutate(label_content.y = str_replace(.$label_content.y, "Geflügel", "Fleisch")) # due to new categories

dat1 <- ubp %>% 
    filter(!duplicated(ubp$meal_name_comp)) %>% 
    group_by(label_content.y) %>% 
    summarise(sum_content = n()) %>% 
    left_join(ubp, ., by = c("label_content.y")) %>% 
    ...
    filter(!duplicated(ubp$meal_name_comp)) # omit duplicates!!
...
p <- ggplot(dat1, aes(y = tot_ubp, x = factor(label_content.y, levels = c("Pflanzlich", "Pflanzlich+", "Vegetarisch", "Fisch", "Fleisch")), fill = label_content.y)) + 
    ...
    scale_x_discrete(breaks = dat1$label_content.y,
                     labels = dat1$xlab_1) +
    labs(x = "Kategorien\n (Anzahl Gerichte)", y = "Gesamtumweltbelastung (UBP / Kategorie))") + # karen wanted to change y-axis
...
p + annotate("text", x = 1:5, y = 10000, label = paste("Median =", round(med$`dat1$tot_ubp`), "UBP"), size = 8) + # annotate median    
...
# save
ggsave("plots/boxplot_ubp_190723_egel.pdf",
       width = 18,
       height = 12,
       dpi = 300,
       device = cairo_pdf)
