...
# attention in online R-Version: problems with the path and encoding (use encoding="ISO8859-1")
...
source("08_theme_plots_180419.R", echo = F)
source("09_function_is_dark_190114.R")
...
p <- ggplot(df, aes(y = pct,x = xlab, fill=factor(label_content,levels=c("Unbekannt","Pflanzlich","Pflanzlich+","Vegetarisch","Fleisch","Hot and Cold")), color=label_color)) +
    ...
    geom_text(aes(label=ifelse(pct>.02,scales::percent(pct, accuracy = 1L), "")), size = 8, position = position_stack(vjust = 0.5))+ 
    annotate("text",x = 1:6,y = 1.03,label=c(text$label2[1], "", text$label2[3],text$label2[2], text$label2[5], text$label2[6]), size=8)+
...
## 6.7 Gender: Female vs. Male (n = 21831)
...
# cluster resp. verpflegungsmuster
df <- reg_visitors %>% 
