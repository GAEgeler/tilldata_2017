---
title: "Script to the the working paper: Transdisziplinäres Experiment in zwei Hochschulmensen: Auswertungen der Menü-Verkäufe"
author: "Gian-Andrea"
date: "Januar 2020"
...
# attenbtion in online R-Version: problems with the path and encoding (use encoding="ISO8859-1")
source("04_load_data_HS17_190128_egel.R",  chdir=TRUE) # sometimes u need to execute that script twice, why?
source("04_load_data_HS15_16_180802_egel.R") # not every regular meal has UBP and EBP points!! => check that
source("08_theme_plots_180419_egel.R", echo = F)
...
# to check wether the script is using offline or online
if(grepl("tilldata_novanimal", getwd())) { # checks if you work online
  path = "online_plots/" # save plots in this folder
  print("FYI: you work on R-Studio Server")
}else{
  path = "plots/" # otherwise in this folder
  print("FYI: you work offline, resp. on the Pool-Server")
}
...
# 3 Einordnung Experiment
## 3.1 zeitliche einordnung
###  total verkaufte menüs 

```{r}
#calculate all sellings per year resp. autumn semester
...
r_2015 <- read_xls(path = here("raw data/Studi_Klassen_Herbstsemester 2015_16.xls"), range = "A32:K33", col_names = F)%>%
    rename(campus = ...1, Mon = ...2, Mon = ...3, Tue = ...4, Tue = ...5, Wed = ...6, Wed = ...7, Thu = ...8, Thu = ...9, Fri = ...10, Fri = ...11)

reg_2015_long <- pivot_longer(r_2015, -campus, names_to = "weekday", values_to = "enrol_stud") %>%
    drop_na() %>%
    mutate(year = 2015) %>%
    select(-.copy)

reg_2015 <- sum(reg_2015_long$enrol_stud) # sum up all the entries

# hs 16_17    
r_2016 <- read_xls(path = here("raw data/Studi_Klassen_Herbstsemester 2016_17.xls"), range = "A28:K29", col_names = F)%>% 
    rename(campus = ...1, Mon = ...2, Mon = ...3, Tue = ...4, Tue = ...5, Wed = ...6, Wed = ...7, Thu = ...8, Thu = ...9, Fri = ...10, Fri = ...11)

reg_2016_long <- pivot_longer(r_2016, -campus, names_to = "weekday", values_to = "enrol_stud") %>%
    drop_na() %>% 
    mutate(year = 2016) %>% 
    select(-.copy)

reg_2016 <- sum(reg_2016_long$enrol_stud) # sum up all the entries

# hs 18_19
r_2018 <- read_xls(path = here("raw data/Studi_Klassen_Herbstsemester 2018_19.xls"), range = "A29:K30", col_names = F) %>% 
    rename(campus = ...1, Mon = ...2, Mon = ...3, Tue = ...4, Tue = ...5, Wed = ...6, Wed = ...7, Thu = ...8, Thu = ...9, Fri = ...10, Fri = ...11)

reg_2018_long <- pivot_longer(r_2018, -campus, names_to = "weekday", values_to = "enrol_stud") %>%
    drop_na() %>% 
    mutate(year = 2018) %>% 
    select(-.copy)


reg_2018 <- sum(reg_2018_long$enrol_stud)

# hs 19_20
# r_2019 <- read_xls(path = here("raw data/Studi_Klassen_Herbstsemester 2019_20.xls"), range = "A27:K28", col_names = F) %>% 
#     rename(campus = ...1, Mon = ...2, Mon = ...3, Tue = ...4, Tue = ...5, Wed = ...6, Wed = ...7, Thu = ...8, Thu = ...9, Fri = ...10, Fri = ...11)
# 
# reg_2019_long <- pivot_longer(r_2019, -campus, names_to = "weekday", values_to = "enrol_stud") %>%
#     drop_na() %>% 
#     mutate(year = 2019) %>% 
#     select(-.copy)
# 
# reg_2019 <- sum(reg_2019_long$enrol_stud)

# predict numbers for 2017, on base of the numbers of 2015 till 2019
df_ <- bind_rows(reg_2016_long, reg_2018_long) %>% #do not take year 2015 and 2019 into account  
...
# very simple, take the mean of all other years
r_2017 <- aggregate(enrol_stud ~ weekday + campus, df_, FUN = mean)
reg_2017_ <- sum(r_2017$enrol_stud)
...
# second stat form the ITC

# not working yet
# reg_2015 <- read_excel("S:/pools/n/N-IUNR-nova-data/00_grundgesamtheit dep N/campus_cards_181220_egel.xlsx", range = "F3:G10", sheet = 2) %>%
#     rename(pop_member = ...1, tot_pop = Frequenz) %>% 
#     drop_na() %>% 
#     filter(pop_member != "Weiterbildungsteilnehmende") %>% 
#     summarise(sum(tot_pop))
# 
# 
# reg_2016 <- read_excel("S:/pools/n/N-IUNR-nova-data/00_grundgesamtheit dep N/campus_cards_181220_egel.xlsx", range = "F3:G10", sheet = 2) %>%
#     rename(pop_member = ...1, tot_pop = Frequenz) %>% 
#     drop_na() %>% 
#     filter(pop_member != "Weiterbildungsteilnehmende") %>% 
#     summarise(sum(tot_pop))
# 
# if (dir.exists("S:/pools/n/N-IUNR-nova-data/00_grundgesamtheit dep N")) {
#   dep_2017 <- read_excel("S:/pools/n/N-IUNR-nova-data/00_grundgesamtheit dep N/campus_cards_181220_egel.xlsx", range = "F3:G10", sheet = 2)
# } else{
#   dep_2017 <- read_excel("raw data/campus_cards_181220_egel.xlsx", range = "F3:G10", sheet = 2)
# }
# 
# #prepare data for plot
# # drop Weiterbildungsteilnehmende
# reg_2017 <- dep_2017 %>%
#     rename(pop_member = ...1, tot_pop = Frequenz) %>% 
#     drop_na() %>% 
#     filter(pop_member != "Weiterbildungsteilnehmende") %>% 
...
# # merge info from above
# df <- menu_tot_line %>%
#     group_by(week, year) %>% 
#     summarise(tot = sum(tot_sold)) %>% 
#     mutate(reg = case_when(year == 2017 ~ reg_2017 * 5,
#                            year == 2016 ~ reg_2016 * 5,
#                            year == 2015 ~ reg_2015 * 5)) %>% 
#     mutate(pot_canteen = tot / reg)
...
p <- ggplot(df, aes(
    y = pot_canteen, x = week, color = as.factor(year), lineshape = as.factor(year)
     ...
     geom_line(size=1.4) + ###always use discrete variables for linetype (factor)
    geom_point(colour= "Black", size=2) +
    guides(color = guide_legend("Herbstsemester", reverse = T)) +
    labs(x = "\nKalenderwochen", y= "Kalkulatorische \"Pick up\"-Rate\n", colour="Mensa") +
    scale_y_continuous(breaks=seq(0, 1, .1), limits=c(0, 1), labels = scales::percent)+
    scale_x_continuous(breaks=seq(38, 52, 1), limits=c(38, 52))+
...
df_actual_7 <- info_orig %>% 
    ...
    dplyr::select(year,label_content,offer)
...
# Group data frame
df_ <- df_t %>%
    group_by(year,offer, label_content) %>%
    summarise(tot = n()) %>%
    mutate(pct = pct(tot))
...
#plot in de
p <- ggplot(df_, aes(y=pct,x=xlab, fill=factor(label_content,levels=c("Unbekannt","Pflanzlich","Pflanzlich+","Vegetarisch","Fleisch","Hot and Cold")), color=label_color)) +
    ...
    geom_text(aes(label=ifelse(pct>.01,scales::percent(pct, accuracy = 1L), "")), size = 8, position = position_stack(vjust = 0.5))+ 
    annotate("text",x = 1:4,y = 1.03,label=c(text$label2[1], "", text$label2[1],text$label2[2]), size=8)+
...
## 3.2 Stichprobe im vergleich zu CampusCard Besitzer
...
# 4 Überblick über Menüverkäufe

## 4.1 Verkäufe nach Menü-Linien 2015, 2016, 2017
...
p <- ggplot(df, aes(y = pct, x = xlab, fill=factor(article_description,levels=c("Local","Kitchen","Green/World","Favorite","Hot and Cold")), color = label_color)) +
    geom_bar(stat="identity",  position = "stack", color=NA, width = .5) + 
    #ggtitle("Verkaufte Men?s: 3. + 4. HSW\n") +
    xlab("Herbstsemester (Kalenderwochen 40 bis 51)") +
    ylab("Verkaufte Gerichte pro Herbstsemester")+
    guides(fill= guide_legend(title = "Menü-Linien", override.aes = list(color = "white")),
           color=F)+
    scale_fill_manual(values = ColsPerCat,
    ...
    geom_text(aes(label = ifelse(df$pct < .02, "", scales::percent(df$pct, accuracy = 1))), size = 8, position = position_stack(vjust = .5))+ # not working properly
...
## 4.2 Wöchentliche Verkäufe 2017 nach Menü-Inhalt
...
# that would be with the mythme0.1 (however does not fit perfectly into the raport)
...
p <- ggplot(data = m_sell, aes(x = xlab, y = pct, 
    ...
    geom_text(aes(label=ifelse(m_sell$pct > .02, scales::percent(m_sell$pct, accuracy = 1), "")), size = 8, position = position_stack(vjust = 0.5))+
    coord_flip()+
    mytheme +
...
#5	Unterschiede zwischen den Mensagästen 


## 5.1 Unterschiede nach Geschlecht und Alter
> age and gender
...
p <- ggplot(df, aes(as.factor(xlab), pct, fill = factor(label_content, c("Unbekannt", "Pflanzlich", "Pflanzlich+", "Vegetarisch", "Fisch","Fleisch", "Hot and Cold")), color = label_color)) + 
    ...
    geom_text(aes(label=ifelse(pct < 0.02,"", scales::percent(pct, accuracy = 1))), size = 8, position = position_stack(vjust = 0.5)) + # omit 0% with ifelse()
        # annotate( 
...
## 5.2 Unterschiede nach Hochschulzugehörigkeit und Geschlecht
>Member and Gender
...
## 5.3 Unterschiede nach Preisniveau Geschlecht und Alter
...
# my colors for the plot
# ColsPerCat = c("keine Angaben" = "#80ccff",
#                "50 bis 64-jährig" = "#fad60d",
#                "35 bis 49-jährig" = "#c5b87c",
#                "26 bis 34-jährig" = "#99f200",
#                "15 bis 25-jährig" = "grey90")
...
p <- ggplot(df_2, aes(xlab, pct, fill = factor(article, c("Kitchen", "Favorite/World", "Hot and Cold")), color = label_color)) + #age_group, c("keine Angaben", "50 bis 64-jährig", "35 bis 49-jährig", "26 bis 34-jährig", "15 bis 25-jährig")
        ...
        xlab("") +
        ylab("\nVerkaufte Gerichte in Prozent\n")+
        guides(fill = guide_legend("", reverse = T, nrow = T),
               color = F)+
        scale_y_continuous(labels=scales::percent) +
        scale_fill_manual(values = ColsPerCat,
                          breaks = attributes(ColsPerCat)$name,
                          labels = c("Kitchen", "Favorite/World", "Hot and Cold"))+
        # scale_x_discrete(limits = c("Frauen\nBasis", "Frauen\nIntervention", "", "Männer\nBasis", "Männer\nIntervention")) + # add some space
...
## 5.4 Unterschiede nach Preisniveau Geschlecht und Hochschuzugehörigkeit
...
##5.5: Unterschiede nach Häufigkeit Mensabesuch: Campus
...
# plot 
p <- ggplot(visiter_freq, aes(x = category,y=pct, fill=shop_description)) +
    geom_bar(stat="identity",colour=NA, position = position_dodge(width = NULL), width = .6)+
    # scale_fill_manual()+
    scale_y_continuous(limits = c(0, .3), breaks = seq(0,.3,.1), labels = scales::percent_format(accuracy = 1L)) + # instead of percent!
    xlab("\nDurchschnittliche Mensabesuche pro Woche") +
    ylab("Anteil Mensabesucher") +
    scale_fill_manual(breaks =  c("Grüental", "Vista"),
                      values =  c("#fad60d","#c5b87c"),
                      labels = c("Grüental", "Reidbach")) +
    guides(fill= guide_legend(title = "Campus")) +
    geom_text(aes(label = scales::percent(pct, accuracy = 1L)), colour = "#000000", position = position_dodge(width = .6),  vjust=-0.25, size= 9) +
    mytheme # see skript 08_theme_plots

# save
ggsave(filename = paste(path, "visit_freq_181128_egel.pdf"),p,
       height = 30,
       width = 54,
       dpi = 300,
       units = "cm",
...
### Häufigkeit Mensabesuche: Geschlecht
...
# plot
p <- ggplot(visiter_freq, aes(x = category, y = pct, fill = gender)) +
    geom_bar(stat="identity",colour=NA, position = position_dodge(width = NULL), width = .6)+
    scale_fill_manual(values =  c("M" = "#6619e6", "F" = "#99f200"),
                      labels = c("F" = "Frauen", "M" = "Männer"))+
    scale_y_continuous(limits = c(0, .35), breaks = seq(0,.35,.10), labels=scales::percent_format(accuracy = 1L)) + #https://stackoverflow.com/questions/53072282/how-to-prevent-scalespercent-from-adding-decimal
    xlab("\nDurchschnittliche Mensabesuche pro Woche") +
    ylab("Anteil Mensabesucher")+
    guides(fill= guide_legend(title = "Geschlecht", override.aes = list(color = "white")))+
    geom_text(aes(label = scales::percent(pct, accuracy = 1)), colour = "#000000", position = position_dodge(width = .6),  vjust=-0.25, size= 9) +
    mytheme

# save
ggsave(paste(path, "visit_freq_gender_181128_egel.pdf"),p,
       height = 30,
       width = 54,
       dpi = 300,
       units = "cm",
...
### Häufigkeit Mensabesuche: Hochschulzugehörigkeit
>visiter frequency by member

```{r}

# by member
visiter <- df_2017 %>%
    group_by(ccrs, member) %>%
    summarise(visit = n()) %>% 
    ungroup() 

# mean of visits for member
aggregate(visiter$visit ~ visiter$member, FUN = mean)
table(visiter$member)
...
# plot
p <- ggplot(visiter_freq, aes(x = category, y = pct, fill = factor(member, levels = c("Studierende", "Mitarbeitende", "Spezialkarten")))) +
    geom_bar(stat="identity",colour=NA, position = position_dodge(width = NULL), width = .6)+
    scale_fill_manual(values =  c("Studierende" = "#99f200", "Mitarbeitende" = "#008099"))+
    scale_y_continuous(limits = c(0, .30), breaks = seq(0,.30,.10), labels=scales::percent_format(accuracy = 1L)) +
    xlab("\nDurchschnittliche Mensabesuche pro Woche") +
    ylab("Anteil Mensabesucher")+
...
# save
ggsave(filename = paste(path, "visit_freq_member_181128_egel.pdf"),p,
       height = 30,
       width = 54,
       dpi = 300,
       units = "cm",
...
# 6 Verpflegungsmuster

## 6.1.Acht Verpflegungsmuster regelmässiger Mensagäste
...
dfList <- list(buffet, no_meat, veg_flex, meat_flex, meat_eat, meat_lovers, alwy_meat)


table <- tibble()
for(i in 1:length(dfList)) {
    if (sum(grepl("reg_visitors", ls())) > 1) {
    description <- reg_visitors %>% 
        filter(cluster == dfList[[i]]$cluster[1]) # filter first datasets
    
    table_0 <- description %>%  
        group_by(gender) %>% 
        summarise(tot = n()) %>% 
        mutate(pct = tot/sum(tot),
               tot_cluster = sum(tot)) %>%  #, cluster = description$cluster)
        ungroup() %>% 
        select(-tot) %>% 
    ...
    table_1 <- description %>%
        ...
        ungroup() %>% 
        select(-tot) %>% 
        rename(variable = member) %>% 
        bind_rows(., table_0)
    
    table_2 <- table_1 %>% 
        mutate(age_m = mean(description[description$age<100,]$age, na.rm = T),
               age_sd = sd(description[description$age<100,]$age, na.rm = T),
               visit_m = mean(description$tot_buy), 
               pct_cluster = round(nrow(description)/nrow(reg_visitors)*100, 0), 
               cluster = dfList[[i]]$cluster[1]) %>% 
        pivot_longer(., -c(variable, cluster), names_to = "sources") 
    
    table_3 <- table_2 %>% 
        filter(variable == "F")
    
    table_4 <- table_2 %>% 
        filter((variable == "Mitarbeitende" & sources == "pct")) %>% 
        bind_rows(.,table_3)
    
    table <- bind_rows(table, table_4) 
    } else{
    print("load data first: see line 1190: Code chunk 18")
  }
  
}

rm(list = c("table_0", "table_1", "table_2", "table_3", "table_4"))
...
```{r}
# prepare data: verpflegungsmuster
df_ <- df_2017
df_ <- df_ %>%
...
df_2 <- df_ %>% mutate(age_group = cut(age, breaks = c(-Inf, 25, 35, 50, 65, Inf), # menuCH age groups
               labels=c("15 bis 25-jährig","26 bis 34-jährig","35 bis 49-jährig","50 bis 64-jährig","keine Angaben"))) %>%
   filter(age_group != "keine Angaben") %>% 
  group_by(member, gender) %>%  # age_groups
  tally() %>% 
  mutate(pct = n / sum(n))
...
dat <- reg_visitors %>% 
    ...
    mutate(info_cluster = recode(cluster, 
                             ...
                             "veg-flexitarians" = "Anteil mit Fleisch: < 1/4", 
...
cairo_pdf("Plots/treemap_cluster_191108_egel.pdf", width = 20, height = 20)
...
## 6.2 Häufigkeit Mensabesuch nach Verpflegungsmuster
...
p <- ggplot(pl, aes(y = value, x = factor(cluster, levels = c("buffet","never meat", "veg-flexitarians", "meat-flexitarians", "meat-eaters", "meat lovers", "always meat")), 
    ...
    scale_fill_manual(values = c("pct_cluster" = "#fad60d","pct_sellings" = "#c5b87c"),
                      ...
                      labels = c("Anteil Mensagäste\nim Verpflegungsmuster\n (n = 19'761)", "Anteil Menü-Käufe\n (n = 990)")) +
    scale_x_discrete(labels = c("Buffetarians","Never Meat", "Veg-flexitarians", "Meat-flexitarians", "Meat Eaters", "Meat Lovers", "Always Meat")) +
    geom_text(aes(label = scales::percent(pl$value, accuracy = 1L)), position = position_dodge(width = .4), vjust = -.25, size = 9) + #check_overlap = TRUE
...
visiter <- reg_visitors %>% 
    mutate(category=cut(tot_buy, breaks = c(-Inf,2,12,24,36,48,60,Inf), labels=c("einmaliger Besuch", "max. 1-mal pro Woche","max. 2-mal pro Woche","max. 3-mal pro Woche","max. 4-mal pro Woche", "max. 5-mal pro Woche","mehr als 5-mal pro Woche"))) %>%
...
ColsPerCat = c(
"max. 1-mal pro Woche" = "grey90",
"max. 2-mal pro Woche" = "#99f200",
"max. 3-mal pro Woche" = "#c5b87c",
"max. 4-mal pro Woche" = "#fad60d",
"max. 5-mal pro Woche" = "#80ccff") #einmaliger Besuch" = "#000000", "mehr als 5-mal pro Woche" = "#ffffff",

visiter2$label_color <- as.factor(sapply(unlist(ColsPerCat)[visiter2$cluster], # takes every label and their belonged color
                                         function(color) { if (isDark(color)) 'white' else 'black' })) # check if color is dark, than give back "white" else "black"

# plot
#factor(cluster, levels = c("buffet", "never meat", "veg-flexitarians", "meat-flexitarians", "meat-eaters", "meat lovers", "always meat"))
p <- ggplot(visiter2_, aes(x = xlab, 
    ...
    scale_x_discrete(limits = rev(c("buffet\nn=19", "never meat\nn=22", "veg-flexitarians\nn=218", "meat-flexitarians\nn=185", "meat-eaters\nn=323", "meat lovers\nn=205", "always meat\nn=18")),
                     labels = rev(c("Buffetarians\nn=19", "Never Meat\nn=22", "Veg-flexitarians\nn=218", "Meat-flexitarians\nn=185", "Meat Eaters\nn=323", "Meat Lovers\nn=205", "Always Meat\nn=18"))) +
    xlab("") +
    ylab("Anteil Mensabesuche in Prozent\n") +
    geom_text(aes(label = ifelse(visiter2$pct<.02,"", scales::percent(pct, accuracy = 1L))), position = position_stack(vjust = .5, reverse = T), size= 8) +
...
## 6.3 Verpflegungsmuster nach Geschlecht und Alter
...
p <- ggplot(pl, aes(y = pct_gender, 
    ...
    xlab("") +
    ylab("\nAnteil Personen mit Verpflegungsmuster\n\n")+
    ...
    scale_y_continuous(labels=scales::percent_format(accuracy = 1L))+
    geom_text(aes(label=ifelse(pct_gender<0.02,"", scales::percent(pct_gender, accuracy = 1))), size = 10, position = position_stack(vjust = 0.5)) + # omit 0% with ifelse()
...
## 6.4 Verpflegungsmuster nach Hochschulzugehörigkeit und Geschlecht
...
pl <- dat %>% 
    ...
    mutate(gender = case_when(.$gender == "F" ~ "Frauen",
                              .$gender == "M" ~ "Männer")) %>% 
    mutate(xlab_ = paste("n", tot_member, sep = " = "),
           xlab = paste(gender, xlab_, sep = "\n"))
...
p <- ggplot(pl, aes(y = pct_member, 
    ...
    facet_wrap(~member, ncol = 1, scales = "free") +
    scale_y_continuous(labels=scales::percent_format(accuracy = 1L))+
    geom_text(aes(label=ifelse(pct_member<0.02,"", scales::percent(pct_member, accuracy = 1))), size = 10, position = position_stack(vjust = 0.5, reverse = T)) + # omit 0% with ifelse()
    scale_color_manual(values = levels(pl$label_color)) +
    mytheme +
    theme(legend.position = "bottom", strip.background =element_rect(fill="grey95"))
...
## 6.5 Vergleich der Mensagäste nach Verpflegungsmustern, Basis-/Interventionswochen, Geschlecht und Menü-Inhalt
...
###### difference in condition between verpflegungsmuster ######
# buffet_no_meat
...
## 5.6 Verpflegungsmuster: statistik
...
# 7 Menü-Wahl von 143 direkt vergleichbaren Mensagästen
>meal offer as influence, not in WP => to do: ask question on cross-validated 

```{r}

# analyse only people with same buyings in both conditions--------
# exclude first people who eat less than 5 times in the canteen
df_1 <- df_2017 %>% 
    filter(ccrs %in% reg_visitors$ccrs) %>% 
    filter(member != "Spezialkarten") %>% # exclude spezialkarten, not necessary
    mutate(label_content = str_replace(.$label_content, "Fisch|Geflügel", "Fleisch")) %>% 
...
# 3. equals (people which bought their same amount in basis as in intervention(50:50)) => 143 persons

gr3 <- df_1 %>% 
    filter(condit == "Basis",pct_visit == 1/2) # drop the duplicates to get information about that group
...
p <- ggplot(pl, aes(y = pct,x = xlab, fill = factor(label_content, c("Unbekannt", "Pflanzlich", "Pflanzlich+", "Vegetarisch", "Fisch", "Fleisch", "Hot and Cold")), color = label_color)) + 
    ...
    scale_fill_manual(values = ColsPerCat,
                      ...
                      labels = c("Unbekannt","Vegan (Fleischersatz)", "Vegan (authentisch)", "Ovo-lakto-vegetarisch", "Fisch", "Fleisch", "Hot & Cold (Buffet)"))+
    scale_color_manual(values = levels(pl$label_color))+
    geom_text(aes(label=ifelse(pct*100>2,scales::percent(pct, accuracy = 1L),"")),size = 8, position = position_stack(vjust = 0.5))+ # omit 0% with ifelse()
    annotate( 
...
#save plot
ggsave(filename = paste(path, "decision_eating_190104_egel.pdf"),p,
       width = 70,
       height = 40,
       dpi = 300,
       units = "cm",
