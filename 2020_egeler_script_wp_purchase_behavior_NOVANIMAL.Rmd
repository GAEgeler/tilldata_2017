...
# to load data menüverkäufe
# errors can be ignored => due to rm()
# attention in online R-Version: problems with the path 
# source("04_load_data_HS17_190128.R", chdir=TRUE, encoding = "latin1") # not reading the data properly, i guess because of the path, load data in chapter 6
source("04_load_data_HS15_16_180802.R", chdir=TRUE, encoding = "latin1") # seems to work and load data properly
...
pl2 <- menu_tot_turnover %>% 
    ...
    mutate(turnover_index = week_turnover/mean(index_turnover_17$sum_turn)) %>% 
    mutate(txt = round(turnover_index, 2)) %>%
    ungroup() %>% 
...
p <-ggplot(pl2, aes(x = factor(week), y = pct_turn, fill = factor(article_description, levels = c("Kitchen", "Green/World","Favorite","Hot and Cold")), color = label_color)) + 
    ...
    guides(fill = guide_legend("Menülinie\n"),
           color = F)+
    facet_wrap(~year)+
    ...
    geom_text(aes(x = factor(week), y = 1.05), label = pl2$txt, size = 6, color = levels(pl2$label_color)[1]) + # seems to work, but the color is black and bold => no other solution found yet
...
ggsave(filename = paste(path, "meal_line_turnover_181203_egel.pdf", sep = ""),p,
       ...
       units="in",
       dpi = 300,
       device = "pdf") # figure 4, has a large size => why?
```
...
p <- ggplot(data = m_sell, aes(x = xlab, y = pct, 
    ...
    geom_text(aes(label=ifelse(pct > .02, scales::percent(round(pct, 2), accuracy = 1), "")), size = 12, position = position_stack(vjust = 0.5))+
    coord_flip()+
    mytheme +
...
## 5.4 Wöchentliche Verkäufe von Fleisch- und Vegi-Gerichten (HS 2017)
...
## 6.1 Unterschiede nach Geschlecht und Alter
> age and gender
...
## 6.2 Unterschiede nach Hochschulzugehörigkeit und Geschlecht
>Member and Gender
...
## 6.4 Unterschiede nach Preisniveau Geschlecht und Hochschuzugehörigkeit
...
visiter_freq <- visiter %>% 
    ...
    mutate(category=cut(visit, breaks = c(-Inf,1,5,12,24,36,48,Inf), 
                        labels=c("1-mal", "2- bis 5-mal", "6- bis 12-mal",
                                 "13- bis 24-mal","25- bis 36-mal","37- bis 48-mal",
                                 "48- bis 60-mal"))) %>%
...
##  do some double checking: it results in the same groups as above
df_ <- df_2017
df_2 <- df_ %>%
...
visiter_freq_2 <- df_2 %>% 
    select(ccrs, shop_description, tot_buy) %>% 
    mutate(category=cut(tot_buy, breaks = c(-Inf,2,5,12,24,36,48,Inf), 
                        labels=c("1-mal", "2- bis 5-mal", "6- bis 12-mal",
                                 "13- bis 24-mal","25- bis 36-mal","37- bis 48-mal",
                                 "48- bis 60-mal"))) %>%
    group_by(shop_description, category) %>% 
    summarise(visit_counts = sum(tot_buy)) %>% # count how hoften a visit occurs, e.g. oneday visitors occur 200 times 
    mutate(pct = visit_counts/sum(visit_counts))
...
p <- ggplot(visiter_freq, aes(x = category, y = pct, fill = gender)) +
    ...
    geom_text(aes(label = scales::percent(round(pct, 2), accuracy = 1)), colour = "#000000", position = position_dodge(width = .6),  vjust=-0.25, size= 11) +
    mytheme
...
```{r chapter 7.1 stats, eval=FALSE}
...
df_2 <- df_ %>% mutate(age_group = cut(age, breaks = c(-Inf, 25, 35, 50, 65, Inf), # menuCH age groups
  ...
  group_by(age_group) %>%  # age_groups, gender
  tally() %>% 
  mutate(pct = n / sum(n))
...
```{r chapter 7.1 stats for types, eval=FALSE}
...
### Stats
```{r chapter 7.1 other stats, eval=FALSE}
# describe clusters 
...
for(i in 1:length(dfList)) {
    ...
    print("load data first: see proper line, above :)")
  }
...
p <- ggplot(pl, aes(y = value, x = factor(cluster, levels = c("buffet","never meat", "veg-flexitarians", "meat-flexitarians", "meat-eaters", "meat lovers", "always meat")), 
    ...
    scale_fill_manual(values = c("pct_cluster" = "#fad60d","pct_sellings" = "#c5b87c"),
                      ...
                      labels = c("Anteil Mensagänger*innen\nnach Verpflegungstypen (100 % = 990)", "Anteil Menüverkäufe\nnach Verpflegungstypen (100 % = 19'761)")) +
    scale_x_discrete(limits = c("buffet","never meat", "veg-flexitarians", "meat-flexitarians", "meat-eaters", "meat lovers", "always meat"),
      labels = c("Buffetarians","Never Meat", "Vegetarian\nFlexitarians", "Meat\nFlexitarians", "Meat Eaters", "Meat Lovers", "Always Meat")) +
...
visiter <- reg_visitors %>% 
    mutate(category=cut(tot_buy, breaks = c(-Inf,2,12,24,36,48,60,Inf), 
                        labels=c("einmaliger Besuch", "höchstens 1-mal", "> 1-mal bis \u2264 2-mal",
                                 "> 2-mal bis  \u2264 3-mal","> 3-mal bis \u2264 4-mal","> 4-mal bis \u2264 5-mal", "> 5-mal"))) %>%
...
visiter2_ <- visiter  %>% 
    ...
    mutate(cluster = factor(cluster, levels = c("buffet", "never meat", 
                                                      "Vegetarian Flexitarians", "Meat Flexitarians", 
                                                      "meat-eaters", "meat lovers", "always meat")))
  
ColsPerCat = c(
"höchstens 1-mal" = "grey90",
"> 1-mal bis \u2264 2-mal" = "#99f200",
"> 2-mal bis  \u2264 3-mal" = "#c5b87c",
"> 3-mal bis \u2264 4-mal" = "#fad60d",
"> 4-mal bis \u2264 5-mal" = "#80ccff") #einmaliger Besuch" = "#000000", "mehr als 5-mal pro Woche" = "#ffffff",
...
p <- ggplot(visiter2_, aes(x = xlab, 
    ...
    scale_x_discrete(limits = rev(c("buffet\nn=19", "never meat\nn=22", "veg-flexitarians\nn=218", "meat-flexitarians\nn=185", "meat-eaters\nn=323", "meat lovers\nn=205", "always meat\nn=18")),
                     labels = rev(c("Buffetarians\n (19)", "Never Meat\n(22)", "Vegetarian Flexitarians\n(218)", "Meat Flexitarians\n(185)", "Meat Eaters\n(323)", "Meat Lovers\n(205)", "Always Meat\n(18)"))) +
    xlab("") +
...
ggsave(filename = paste(path, "visit_freq_cluster_190525_egel.pdf"),p,
       ...
       width = w_1,
       dpi = 300,
       device = cairo_pdf) # figure 16

```

## 7.4 Häufigkeit Mensabesuche nach Geschlecht und Alter
...
visiter <- reg_visitors %>% 
    mutate(category=cut(tot_buy, breaks = c(-Inf,2,12,24,36,48,60,Inf), 
                        labels=c("einmaliger Besuch", "höchstens 1-mal", "> 1-mal bis \u2264 2-mal",
                                 "> 2-mal bis  \u2264 3-mal","> 3-mal bis \u2264 4-mal","> 4-mal bis \u2264 5-mal", "> 5-mal"))) %>%
    group_by(member, gender, category) %>% 
    summarise(visit_counts=n()) %>% 
    mutate(pct=visit_counts/sum(visit_counts)) %>% 
    ungroup()
...
visiter2_ <- visiter  %>% 
    ...
    summarise(tot = sum(visit_counts)) %>% 
    ungroup() %>% 
    left_join(., visiter, by = c("gender", "member")) %>% 
    mutate(gender = ifelse(.$gender == "F", "Frauen", "Männer")) %>% 
    mutate(label = paste("(", tot, ")", sep = ""),
           xlab = paste(gender, label, sep = "\n"))  
...
visiter2_$label_color <- as.factor(sapply(unlist(ColsPerCat)[visiter2_$category], # takes every label and their belonged color
                                         function(color) { if (isDark(color)) 'white' else 'black' })) # check if color is dark, than give back "white" else "black"
...
p <- ggplot(visiter2_, aes(x = xlab, 
    ...
    xlab("") +
    ylab("\ndurchschnittliche Mensabesuche pro Woche\n") +
    geom_text(aes(label = ifelse(pct<.02,"", scales::percent(round(pct, 2), accuracy = 1L))), position = position_stack(vjust = .5, reverse = T), size= 12) +
    # annotate("text", x = 1:7, y = 1.05, label = text$label, parse = T, size = 9) +
    facet_wrap(~member, ncol = 1, scales = "free") +
    mytheme +
    theme(legend.position = "bottom", strip.background =element_rect(fill="grey95"))

ggsave(filename = paste(path, "visit_freq_member_gender_freq_200708.pdf"), p, 
       height = h_15,
       width = w_1,
       dpi = 300,
...
p <- ggplot(pl, aes(y = pct_gender, 
    ...
    xlab("") +
    ylab("\nAnteil Personen mit Verpflegungstypen\n\n")+
    guides(fill = guide_legend(title = "Verpflegungstypen\n"), 
           color = F) +
    scale_fill_manual(values = ColsPerCat, 
                      breaks = c("buffet",
                                 "never meat",
...
## 7.6 Verpflegungstyp nach Hochschulzugehörigkeit und Geschlecht
