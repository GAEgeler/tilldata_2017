...
# check if there is a folder "plots"
...
df_angebot_b <- info_orig %>%
    ...
    reshape2::dcast(formula = date+shop_description ~ label_content, value.var = "label_content", fun.aggregate = length) %>%
    rename(unbekannt = `NA`) %>% 
...
```{r table 4 and table 10, eval=FALSE}
...
text <- group_by(df, year) %>% 
    summarise(tot=sum(tot_sold)) %>%
    mutate(tot2 = format(tot, big.mark = "'", scientific = F)) %>% # add thousand separator, however don't goes with ggplot2
...
# plot data
# order geom_text: https://github.com/tidyverse/ggplot2/issues/3612
...
p <- ggplot(pl2, aes(x = factor(week), y = pct_turn, fill = factor(article_description, levels = c("Kitchen", "Green/World","Favorite","Hot and Cold")), color = label_color)) + 
    ...
    geom_text(aes(x = factor(week), y = 1.05), label = pl2$txt, size = 8, color = levels(pl2$label_color)[1]) + # seems to work, but the color is black and bold
...
ggsave(filename = paste(path, "meal_line_turnover_181203_egel.pdf", sep = ""),p,
       ...
       height = h_1,
       dpi = 300,
       device = cairo_pdf) # figure 4
```



```{r anova with interaction 5.3, eval=FALSE}
...
p <- ggplot(data = m_sell, aes(x = xlab, y = pct, 
  ...
  scale_x_discrete(limits = rev(c("Alle\nn = 26'340" , "Basis\nn = 13'218", "Intervention\nn = 13'122")))+
    scale_fill_manual(values = ColsPerCat,
                      breaks = attributes(ColsPerCat)$name,
...
p <- ggplot(df_, aes(y = pct,x = as.factor(xlab), fill = factor(label_content, c("Unknown", "Pflanzlich", "Pflanzlich+", "Vegetarisch", "Fisch", "Fleisch", "Hot and Cold")), color = label_color)) + 
    ...
    annotate( 
        "text",x = 1:12, y = 1.03, label = text$label2,parse=T, size = 9) +
    mytheme # see skript 08_theme_plots
...
#6	Unterschiede bei der Menüwahl zwischen den Mensagästen 
...
aggregate(visit_counts ~ category + member + gender, data = visiter_freq, FUN = sum)
...
# 7 Verpflegungstypen der Mensagäste
...
# reg_visitors see chuck 20 and 22
...
pl <- left_join(purchase, cluster_dat, by = "cluster") %>% 
    # mutate(xlab = paste(cluster, count_cluster, sep = "\n")) %>%     
    select(-count_cluster, -tot_sell) %>% 
    pivot_longer(-c(cluster), values_to = "value", names_to = "variable") # long format
...
p <- ggplot(pl, aes(y = value, x = factor(cluster, levels = c("buffet","never meat", "veg-flexitarians", "meat-flexitarians", "meat-eaters", "meat lovers", "always meat")), 
    ...
    scale_x_discrete(limits = c("buffet","never meat", "veg-flexitarians", "meat-flexitarians", "meat-eaters", "meat lovers", "always meat"),
      labels = c("Buffetarian\n(19)","Never Meat\n(22)", "Vegetarian\nFlexitarian\n(218)", "Meat\nFlexitarian\n(185)", "Meat Eater\n(323)", "Meat Lover\n(205)", "Always Meat\n(18)")) + # I added the cluster size manually
...
ColsPerCat = c(
"6- bis 11-mal" = "#99f200",
"12- bis 23-mal" =  "#008099",
"24- bis 35-mal" = "#fad60d",
"36- bis 47-mal" = "#c5b87c",
"48- bis 60-mal" = "#262626")
...
# vegan with and without locals
veg <- df_agg %>% 
  # filter(., str_detect(.$article_description, "Local ", negate = TRUE)) %>% 
  # without locals deactivate comment #
  filter(., str_detect(.$label_content, "Pflanzlich+")) %>% 
  mutate(orig = if_else(label_content == "Pflanzlich", "Fleischersatz", "authentisch")) %>% 
  group_by(orig) %>% 
  summarise(tot_sold = n()) %>% 
  mutate(pct = pct(tot_sold)) %>% 
  ungroup()
