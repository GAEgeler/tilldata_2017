...
pack <- c("dplyr", "readr", "readxl", "stringr")
...
trans_dat1 <- read_xlsx("raw data/Kaufverhalten_2018-09-27_V01.xlsx", sheet = 5, col_names = T, trim_ws = T) %>% # trim_ws removes white space
    ...
    mutate(ccrs = parse_integer(.$ccrs), # parse to integer otherwise r saves it as numeric (e.g. 2.67e3)
           transaction_id = parse_integer(.$transaction_id),
           card_num = parse_integer(.$card_num))

# shop location
till <- read_xlsx("raw data/ZHAW_Tills.xlsx") %>% # good for small datasets
...
art_info <-  read_excel("raw data/ZHAW_Articles.xlsx") %>%
    select(id, code, description) %>%
    rename(article_id = id, art_code = code, art_description = description) %>%
    mutate(article_id = parse_integer(.$article_id))

# load price levels => rabatt information
price_lev <- read_excel("raw data/ZHAW_Pricelevels.xlsx") %>%
    select(id, code, description) %>%
    rename(pricelevel_id = id, price_code = code, price_descript = description)

# load article price information
art_price <- read_excel("raw data/ZHAW_prices.xlsx", sheet = 1, col_names = T) %>% # conversts some variables into characters => in the excel file is everything stored as text
...
trans_pay <- read_delim("raw data/ZHAW_Trans_Payments.csv", delim = ';', trim_ws = T) %>%
...
trans_pay2 <- read_delim("raw data/ZHAW_payments_180313.csv", delim = ';', trim_ws = T) %>%
    select(transaction_id, payment_id, amount) %>%
    rename(payment_id = payment_id, price_p = amount)

# check difference
df_diff <- anti_join(trans_pay, trans_pay2, by = "transaction_id") # they differ by 3990 Transaktions

# load payments information
pay_method <- read_delim("raw data/ZHAW_payments_171214.csv", delim = ';',trim_ws = T) %>%
    select(id, code, description) %>%
