...
Von insgesamt 93 verschiedenen Gerichten, welche über zwei Mensazyklen^[ein Zyklus besteht aus 6 Wochen, danach wiederholen sich die Gerichten] verteilt angeboten wurden, enthielten `r round(45/93, digits = 0)` Prozent (n = 45) der Gerichte Fleisch. Ein Drittel (n = 31) der Gerichte wurde vegetarisch und die restlichen `r round(16/93, digits = 0)` Prozent (n = 16) ausschliesslich pflanzlich angeboten.
<br>
...
t <- filter(info_orig, !grepl("Hot and Cold", info_orig$label_content)) %>% group_by(label_content, condit) %>% summarise(tot = n()) %>% ungroup() %>% group_by(condit) %>% mutate(pct = tot/sum(tot))  

# show aggregated offer without locals (N = 360)
p <- filter(info_orig, !grepl("Local ", info_orig$article_description) & !grepl("Hot and Cold", info_orig$label_content)) %>% group_by(label_content, condit) %>% summarise(tot = n()) %>% ungroup() %>% group_by(condit) %>% mutate(pct = tot/sum(tot))
...
t <- drop_na(t) %>% group_by(label_content) %>% summarise(tot = sum(tot)) %>% mutate(pct = tot/sum(tot)) 
p <- group_by(p, label_content) %>% summarise(tot = sum(tot)) %>% mutate(pct = tot/sum(tot))
...
tp <- drop_na(t) %>% select(-pct) %>% bind_cols(., p[c("tot")]) %>% rename( real_offer = tot, planed_offer = tot1)

# basis
tp <- filter(tp, condit == "Basis") %>% mutate(pct_exp = planed_offer / sum(planed_offer), 
                                               pct_real = real_offer / sum(real_offer))
chi_s2 <- chisq.test(tp$real_offer, p = tp$pct_exp) # check if expected frequencies are over 5
fisher.test(tp[ ,3:4]) # for exact p-value, it differs strongly form the p-value from chi-square 

#interventionswochen
tp <- filter(tp, condit == "Intervention") %>% mutate(pct_exp = planed_offer / sum(planed_offer), pct_real = real_offer / sum(real_offer))
chi_s3 <- chisq.test(tp$real_offer, p = tp$pct_exp) # check if expected frequencies are over 5
fisher.test(tp[ ,3:4]) # for exact p-value, it differs strongly form the p-value from 

```

```{r,fig.cap="\\label{fig:fig2} Die Abbildung 2 zeigt das geplante (480) und angebotene Angebot (600) für ein ganzes Herbstsemester (60 Tage) und zwei Hochschulmensen nach den fünf Menü-Inhalten", fig.height=14, fig.width=25}
...
Das geplante Angebot und das tatsächliche Angebot im HS 2017 unterscheiden sich in den Menü-Inhalten^[Fleisch oder Fisch, Vegetarisch, Vegan (authentisch) und Vegan (Fleischsubstitut)] nicht statisch ($\chi^2$(3) = `r round(chi_s$statistic[[1]], digits = 3)`, *p* = `r round(chi_s$p.value[[1]], digits = 2)`). Auch beim näheren Betrachten unterschieden sich die geplanten und tatsächlichen Angeboten für das HS 2017 in den Menü-Inhalten zwischen den Basis- ($\chi^2$(3) = `r round(chi_s2$statistic[[1]], digits = 3)`, *p* = `r round(chi_s2$p.value[[1]], digits = 2)`) und Interventionswochen ($\chi^2$(3) = `r round(chi_s3$statistic[[1]], digits = 3)`, *p* = `r round(chi_s3$p.value[[1]], digits = 2)`) nicht. 
<br>
Wird das tatsächliche Angebot näher beleuchtet, wurden in über alle Interventionenwochen (*n* = 238) 40 Prozent fleischhaltige, knapp einem Drittel vegetarische und 27 Prozent vegane Gerichte angeboten. In den 12 Basiswochen entiehlten $2/3$ Prozent des Angebots Fleisch oder Fisch. Vegetarische Gerichte machten rund 29 Prozent und vegane Gerichte knapp 1 Prozent aus.
<br>
Im Vergleich zu den vergangenen Herbstsemestern 2015 & 2016 wuchs das vegetarische und vegane Angebot im Herbstsemester 2017 von `r ((30*.2)+30)/90*100` Prozent auf knapp 47 Prozent, wobei das Hot and Cold-Buffet nicht als vegetarisches Angebot mitgezählt wurde. 
<!-- überprüfe nochmals ob das so stimmt -->
\newpage

#### 4.2 Was wurde verkauft?

Die totalen Verkaufszahlen des Herbstsemesters 2017 haben sich im Vergleich zu den zwei HS 2015 & 2016 nur marginal verändert. Die Abbildung \ref{fig:fig3} zeigt alle verkauften Gerichten nach Menü-Inhalt (vegan, vegetarisch, Fleisch und Buffet), welche während des Quasi-Experiments verkauft wurden.

```{r, echo=F, fig.cap="\\label{fig:fig3} Die Abbildung 3 zeigt alle verkauften Gerichten (N = 26'234) aufgeteilt nach den fünf Menü-Inhalten über die 12 Wochen", fig.height=14, fig.width=25}
...
#### 4.4 Lassen sich vegetarische Gerichte auch teuer verkaufen?

Sind die aktuellen Menü-Linien tatsächlich ein veraltetes Konzept? Nebst der Änderung im Angebot, war eine weitere Intervention die Gerichte über neutrale Menü-Gefässe sog. Menü-Linien zu vertreiben. Für das Quasi-Experiment wurde die Menü-Linie Green zu World unbenannt. Die Menü-Inhalte wurden randomisiert über diese drei Menü-Linien angeboten. Die Abbildung \ref{fig:fig4} vergleicht die Verkaufszahlen gemessen an den Menü-Linien der Jahren 2015, 2016 und 2017.
...
Die Abbildung \ref {fig:fig4} zeigt, dass es zwischen den drei Herbstsemestern klare Unterschiede in den Verkauszahlen zwischen den Menü-Linien gab. Die Grafik zwar lässt keine Schlüsse über deren Menü-Inhalt zu, aber die randomisierte Ausgabe der Gerichte konnte aufzeigen, dass es einen Einfluss auf die Menü-Wahl genommen hat. So wurden auch vegetarischen Gerichte auf der teureren Menü-Linie «Kitchen» problemlos von den Gästen gekauft.


###	5. Diskussion

Nach unserem Wissen wurde noch kein Real-Labor-Experiment durchgeführt, welches die Reaktion der Gäste auf ein ressourcenschonenderes Angebot aufzeichnete. Es gibt einige Untersuchungen wobei mittels Quasi-Experimenten getestet wurde wie die Mensagäste auf bestimmte Label (z. B. zu gesunder Ernährung oder umweltfreundlichen Gerichten) [@hoefkens ; @spaargaren] oder auf soziale oder injuktive Normen [@ collins] reagierten.  
Zusammengefasst zeigen die Ergebnisse dieser Studie, dass eine Erhöhung des vegetarischen und veganen Angebots in Kombination mit einer randomisierten Ausgabe zu einer Senkung des Fleischkonsums führen. Ob nun die randomisierte Ausgabe oder die quantitative Erhöhung von vegetarische und vegane Gerichten für diesen Effekt zuständig ist, bedarf es an weiteren Untersuchungen. Zudem basieren die gezeigten Ergebnisse auf aggregierte Verkaufszahlen. Inwiefern die Intervention auch tatsächlich einen Einfluss auf das Wahl-Verhalten nimmt, soll weiter Untersucht werden (besser schreiben).
<br>
Schreibe nochmals so viel mit mehr literatur

gehe noch auf die einzelnen ergebnisse näher ein!!
...
eröhung von quantitativem veg2 menü-angebot scheint die fleischverkaufszahlen zu senken
es gab keine verkaufseinbrüche wie von praxispartnern angenommen mut für weitere pilozprojekte dieser art
vegan nur autentisch verkaufbar
es lassen sich auf teurere mit besseren Ingredienzen auf vegetarische/veganen ebene verkaufen
...
Folgende Skripte wurden für diesen Bericht verwendet:
- 04_load_data_180802_egel
- 05_load_add_data_180802_egel
- 08_theme_plots_180419_egel
 
Die Skripte und dieser Bericht (mit allen Berechnungen) sind auf [github](https://github.com/GAEgeler/tilldata_2017) online verfügbar. 
